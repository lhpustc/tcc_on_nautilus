...
void *
nk_assist_malloc (ulong_t order)
{
    void *block = 0;
    struct kmem_block_hdr *hdr = NULL;
    struct mem_reg_entry * reg = NULL;
...
void * nk_tcc_realloc(void *p, size_t newsize) 
{
    struct kmem_block_hdr *hdr = NULL;
    //nk_vc_printf("\n\nnew size to allocate is %u\n", newsize);
    ulong_t neworder = ilog2(roundup_pow_of_two(newsize));
    //nk_vc_printf("new order = %u\n", neworder);
 
    if (neworder < MIN_ORDER) { neworder = MIN_ORDER; }
 
    hdr = block_hash_find_entry(p);
    if (!hdr) { 
      nk_vc_printf("FAIL to find pointer %p\n" ,p);
      return 0 ;
       //FAIL - allocator is unaware of block
    }
   
    if (hdr->order == neworder) {  
       return p; // do nothing - still plenty of room in block
    }
    //struct kmem_block_hdr * hdr_p = block_hash_find_entry(p);
    //nk_vc_printf("order_P = %u\n", hdr_p->order);
    size_t size_p = 1<<hdr->order;



    if (hdr->order < neworder) {
       // EXPAND: malloc a new block, copy data to new block, free old block, return new block
      void* new = malloc (newsize);
      if(!new ){
        nk_vc_printf("fail to reallocate\n");
        return 0;
      }
      //nk_vc_printf("orignial data size is  %u\n\n", size_p);
      //nk_vc_printf("neworder is  %u\n", neworder);
      memcpy(new ,p, size_p);
      //nk_vc_printf("info conpare:  %p\n", p);
      //nk_vc_printf("info conpare:  %p\n", new);
      free(p);
      return new;
    ...
    if (hdr->order > neworder) { 
      ...
      memcpy(new ,p, newsize);
      free(p);
      return new;
       // SHRINK: malloc, copy (only to newsize), free old, return new
    }
}
